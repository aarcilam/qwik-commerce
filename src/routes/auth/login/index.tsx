...
export const useFormAction = formAction$<LoginForm, LoggedUsedResponseData>(
  async (values) => {
    // Runs on server
    try {
      const userService = new UserService();
      const loggedUser: User | null = await userService.getUserByEmail(
        values.email
      );
      if (loggedUser == null) return;
      const validatePass = await userService.validateUser(
        loggedUser,
        values.password
      );
      if (!validatePass) {
